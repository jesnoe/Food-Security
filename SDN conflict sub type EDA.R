...
SDN_high_violence_index <- which(names(lm_conflict_SDN$residuals) %>% as.numeric %in% which(IPC_SDN_phase3_long_lagged$Area %in% SDN_Area_ordered_by_n_events[1:7]))
lm_conflict_SDN$residuals[SDN_high_violence_index] %>% summary
mean((lm_conflict_SDN$residuals)^2)
mean((lm_conflict_SDN$residuals[SDN_high_violence_index])^2) # 0.004328112
lm_conflict_SDN_y_bar <- mean(lm_conflict_SDN$model$Phase_3above_ratio[SDN_high_violence_index])
1-sum((lm_conflict_SDN$residuals[SDN_high_violence_index])^2)/sum((lm_conflict_SDN$model$Phase_3above_ratio[SDN_high_violence_index]-lm_conflict_SDN_y_bar)^2) # R^2: 0.8073256
lm_conflict_diff_SDN$residuals[SDN_high_violence_index] %>% summary
mean((lm_conflict_diff_SDN$residuals)^2)
mean((lm_conflict_diff_SDN$residuals[SDN_high_violence_index])^2) # 0.006769709
lm_conflict_SDN_high_violence$residuals %>% summary
lm_conflict_diff_SDN_high_violence$residuals %>% summary
mean((lm_conflict_SDN_high_violence$residuals)^2) # 0.004122871
mean((lm_conflict_diff_SDN_high_violence$residuals)^2) # 0.006010389

ggplot(data.frame(x=1:length(SDN_high_violence_index), residuals=lm_conflict_SDN$residuals[SDN_high_violence_index])) +
  geom_point(aes(x=x, y=residuals)) + ylim(-0.2, 0.2) + ggtitle("Residuals in highly conflicted areas of global model (SDN)") +
  geom_abline(slope=0, intercept=mean((lm_conflict_SDN$residuals[SDN_high_violence_index])^2))

ggplot(data.frame(x=1:length(SDN_high_violence_index), residuals=lm_conflict_SDN_high_violence$residuals)) +
  geom_point(aes(x=x, y=residuals)) + ylim(-0.2, 0.2) + ggtitle("Residuals of local model (SDN)") +
  geom_abline(slope=0, intercept=mean((lm_conflict_SDN_high_violence$residuals)^2))
...
# write.csv(IPC_SDN_phase3_long_lagged_rainfall, "Food Security/IPC_SDN_phase3_long_lagged_rainfall.csv", row.names = F)
...
lm_conflict_SDN <- lm(Phase_3above_ratio~., data=IPC_SDN_phase3_long_lagged[,4:17]) # without IPC_t_2
lm_conflict_SDN %>% summary
lm(Phase_3above_ratio~., data=IPC_SDN_phase3_long_lagged[,4:18]) %>% summary # with IPC_t_2 (insignificant)
lm_conflict_diff_SDN <- lm(Phase_3above_ratio~., data=IPC_SDN_phase3_long_lagged %>% select(-(Area:month), -(n_events_Battles_explosions:fatalities_etc.)))
lm_conflict_diff_SDN %>% summary
...
# ts plots
lagged_months <- 1
IPC_SDN_provinces_long %>% 
  mutate(year=as.Date(paste(Month, Year, "01"), format="%m %Y %d")) %>% 
  ggplot() + ylim(0, 1) +
  geom_line(aes(x=year, y=Phase_3above_ratio, group=Area, color=Area))
ggsave("Food Security/Figs/ts plots/SDN/food insecurity ts plot SDN.png", scale=1)

conflict_SDN_monthly_aggr_by_type <- lagged_reg_data_list_CAF[[lagged_months]]$conflict_sub_NAT_aggr %>% group_by(year, month, event_type) %>%
  summarize(n_events = sum(n_events),
            fatalities = sum(fatalities)) %>% 
  arrange(year, month) %>% 
  mutate(year=as.Date(paste(month, year, "01"), format="%m %Y %d"))

conflict_SDN_monthly_aggr_by_type %>% 
  ggplot() +
  geom_line(aes(x=year, y=fatalities, group=event_type, color=event_type))
ggsave("Food Security/Figs/ts plots/SDN/conflict fatalities ts plot SDN.png", scale=1)

conflict_SDN_monthly_aggr_by_type %>% 
  ggplot() +
  geom_line(aes(x=year, y=n_events, group=event_type, color=event_type))
ggsave("Food Security/Figs/ts plots/SDN/conflict n_events ts plot SDN.png", scale=1)
...
ggsave("Food Security/Figs/ts plots/SDN/disaster affected ts plot SDN.png", scale=1)

disaster_SDN_monthly_aggr_by_type %>% 
  ggplot() +
  geom_line(aes(x=year, y=deaths, group=type, color=type))
ggsave("Food Security/Figs/ts plots/SDN/disaster deaths ts plot SDN.png", scale=1)

disaster_SDN_monthly_aggr_by_type %>% 
  ggplot() +
  geom_line(aes(x=year, y=n_disasters, group=type, color=type))
ggsave("Food Security/Figs/ts plots/SDN/disaster n_disasters ts plot SDN.png", scale=1)
