...
reg_data_i <- IPC_SDN_provinces[,c(1, IPC_ncol:(IPC_ncol-2))] %>% 
  ...
  # left_join(disaster_SDN_monthly_events[,c(1, disaster_ncol:(disaster_ncol-2))], by="Area") %>% 
  # left_join(disaster_SDN_monthly_affected[,c(1, disaster_ncol:(disaster_ncol-2))], by="Area") %>%
  # left_join(disaster_SDN_monthly_deaths[,c(1, disaster_ncol:(disaster_ncol-2))], by="Area")
...
lagged_reg_data$sorghum <- time_since_harvest(IPC_SDN_year_month$Month[5], "sorghum", SDN_harvest)
...
for (i in 1:3) {
  ...
  reg_data_i <- IPC_SDN_provinces[,c(1, IPC_col_index:(IPC_col_index-2))] %>% 
    ...
    # left_join(disaster_SDN_monthly_events[,c(1, disaster_col_index:(disaster_col_index-2))], by="Area") %>% 
    # left_join(disaster_SDN_monthly_affected[,c(1, disaster_col_index:(disaster_col_index-2))], by="Area") %>% 
    # left_join(disaster_SDN_monthly_deaths[,c(1, disaster_col_index:(disaster_col_index-2))], by="Area")
  reg_data_i$month_diff <- as.numeric(as.Date(paste(IPC_SDN_year_month$Year[6-i], IPC_SDN_year_month$Month[6-i], 1), format="%Y %m %d") -
                                        as.Date(paste(IPC_SDN_year_month$Year[5-i], IPC_SDN_year_month$Month[5-i], 1), format="%Y %m %d")) %/% 30
  
  reg_data_i$sorghum <- time_since_harvest(IPC_SDN_year_month$Month[14-i], "sorghum", SDN_harvest)
